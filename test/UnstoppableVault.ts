...
describe('[Challenge] Unstoppable', function () {
  ...
  let token: Contract, vault: Contract, receiverContract: any;
  ...
  it('Execution', async function () {
    ...
    // Show it's possible for someUser to take out a flash loan
    receiverContract = await (
      await ethers.getContractFactory('ReceiverAttacker', player)
    ).deploy(vault.address);

    await receiverContract.executeFlashLoan(ethers.utils.parseEther('100000'));
  });
